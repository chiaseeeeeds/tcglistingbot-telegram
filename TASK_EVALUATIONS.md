...
- date: 2026-04-13
- goal: unblock strict Phase 1 launch scope by adding real catalog import paths for One Piece and Japanese Pokémon
- outcome: added `scripts/import_onepiece_official.py` using the official One Piece EN and JP cardlist pages, and added `scripts/import_pokemon_jp_official.py` using the official Japanese Pokémon search API plus detail pages for printed set codes and collector numbers
- validation: `python -m py_compile` passed for both importers; smoke runs imported one One Piece series (`285` bilingual rows from PRB02) and one Japanese Pokémon result page (`39` rows from set code `M4`) into the live database
- what was weak: the strict Phase 1 blocker has shifted from missing importer code to importer runtime time, because the full Japanese Pokémon crawl is long and still needs to run to completion before launch-scope support can honestly be marked done
- follow-up: let the long-running One Piece + Japanese imports finish, verify final card counts, then run real Telegram OCR/listing/claim QA on both new catalog paths

## 2026-04-13 — JP Importer Crash Fix And Pricing Latency Guard
- task: continue Phase 1 completion work by hardening the live Japanese catalog importer and removing an avoidable pricing slowdown in the seller flow
- outcome: added `PRICECHARTING_SCRAPE_FALLBACK_ENABLED` with a default-off path so `/list` no longer waits on blocked PriceCharting scraping when no API token is configured; hardened `scripts/import_pokemon_jp_official.py` so numberless official JP detail pages are skipped with structured logs instead of crashing the whole import; then advanced the JP import from page 25 through page 35 successfully
- validation: `.venv/bin/python -m py_compile config.py services/pricecharting.py handlers/listing.py handlers/auctions.py scripts/import_pokemon_jp_official.py` passed; a live price probe returned Pokémon TCG API references for a real Pokémon card in about 3.6 seconds; `scripts/phase1_catalog_audit.py` now reports `pokemon_total=21273` and `jp_named_total=1358`; `scripts/phase1_match_smoke.py` still passes `12/12` for both One Piece and Japanese Pokémon
- what went well: the real Japanese catalog blocker turned out to be importer brittleness, not source availability, and the fix was surgical; the pricing latency improvement also made the live seller flow more truthful because it now fails fast instead of pretending PriceCharting is usable without credentials
- what was weak: detached local polling is still not trustworthy in this environment, so live Telegram QA is still operationally fragile even though the bot logs show clean startup; the full JP crawl is still far from complete and must continue beyond page 35
- follow-up: keep pushing the JP importer to completion under a supervised session, validate live `/list` against JP + One Piece once the bot runtime is stable, and then complete strict linked-discussion claim/payment QA before declaring Phase 1 done
- continuation note: after advancing the JP crawl to page 67, the remaining failure mode is upstream 403 rate-limiting on `resultAPI.php` page fetches; parser and per-card fetch resilience improved, but completion now depends on waiting out or routing around the official site throttle
- deeper continuation note: probing the site directly showed page 68 itself still returned 200 when fetched fresh, so the real bug was request burstiness; after changing the importer to use browser-like headers, per-detail jitter, and longer 403 backoff at low concurrency, the crawl advanced cleanly from page 68 through page 85 and the live audit rose to `jp_named_total=3291`
- deeper continuation note 2: the slower crawl exposed a second root cause at the database boundary — one long-lived Postgres connection was timing out during extended import runs. Switching to per-page DB connections removed that failure mode and let the crawl advance through page 92, raising `jp_named_total` to 3564.
- deeper continuation note 3: once throttling and DB timeout issues were fixed, a third failure mode appeared — duplicate card identities within the same JP page payload. Deduping source rows before insert resolved the unique-index conflict and let the crawl continue through page 110, raising `jp_named_total` to 4230.
- runtime continuation note: the apparent bot/import-loop instability was partly a launch-mode problem. Detached background starts died silently here, while PTY runs stayed alive and the JP checkpoint advanced from 220 to 221 immediately; that means the local process model is still a blocker, not just the importer itself.
- listing UX note: the apparent 'death' after replying `done` was most likely long-running OCR rather than a handler crash. Added an immediate progress reply plus timing logs in `finalize_photo_batch(...)`, then restarted the bot successfully.
- date: 2026-04-14
- goal: implement the full buyer payment-proof workflow requested by the user
- outcome: added `claims.payment_reference`, created `claim_payment_proofs`, added `/pay` DM selection + screenshot upload handling, forwarded proofs to sellers with approve/reject callbacks, and threaded payment references into fixed-price claims, queue promotion, and auction award notifications
